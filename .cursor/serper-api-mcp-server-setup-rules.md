# Serper API MCP Server Implementation Guide

You are working on implementing the Serper API MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.
...
When a client provides their own Serper API API key:
...
Flow:
  1. Client connects with their Serper API API key
  2. MCP server uses client's API key to call Serper API API
  3. No payment required
  4. Client pays Serper API directly (not the MCP server)
```

**Use Case**: Clients who already have a Serper API subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Serper API API key to call the API
  5. Client pays the MCP server (not Serper API)
```

**Use Case**: Pay-per-use clients without their own Serper API subscription
...
**Required**:
- `SERPER_API_KEY`: Server's internal Serper API API key (used when clients pay via 402)
...
SERPER_API_KEY=your_serper-api_api_key_here
...
```json
{
  ...
  "tags": ["serper api", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Serper API API with retry support. Example pattern:
...
def call_serper_api_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Serper API API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Serper API API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_serper_api(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Serper API for [specific data type].
    ...
    try:
        # The call_serper_api_api function already has retry logic
        results = call_serper_api_api(
...
```python
@mcp.tool()
async def create_serper_api_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Serper API.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_serper_api(query="example", limit=5)
   ```
...
- Check the Serper API API documentation: https://serper.dev/docs
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Serper API's capabilities easily accessible to AI agents through standardized MCP tools. 