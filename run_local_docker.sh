...
# Script to build and run the Serper API MCP Server locally in Docker
...
IMAGE_NAME="serper-api-mcp-server"
CONTAINER_NAME="serper-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Serper API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Serper API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^SERPER_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Serper API API key:${NC}"
...
echo
echo -e "${GREEN}✅ Serper API MCP Server is running!${NC}"
echo
